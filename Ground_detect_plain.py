...
#step.0 ファイルイン　ver.1 ZHU 25.10.19
def filein(fn):
    data = []
...
#tool.0.0 四近傍点idx ver.3 ZHU 25.10.23
def c4(i):
...
#tool.0.1 符号付角度計算 ver.3 ZHU 25.11.19
...
#tool.0.2 符号付相対高度 ver.1 ZHU 25.10.19
def dis(i1,i2):
...
#tool.2 川中心四近傍 ver.2 ZHU 25.11.19
def river(i): 
    c = c4(i)
    for j in c:
        if j is None:continue
...
#tool.3 建物中心四近傍 ver.2 ZHU 25.11.19
def building_city(i):
    c=c4(i)
    for j in c:
        if j is None:continue
...
#use xy to get index ver.3 ZHU 25.11.19
idx_by_xy={}
for i in range(len(data)):
